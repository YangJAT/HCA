...
MAGIC_impute<-function(datafilt, knn = 30, t = 3, npca = 50,
                       only_marker = TRUE,organism = 'human',
                       ncore= 10){
...
#' @examples
#'
infer_tumor <- function(datafilt = datafilt,
                       cnadata = cnadata,
                       thres_sig = 0.005,
                       thres_cor = 0.5,
                       organism = 'human',
...
#' @import Matrix
#' @examples
anno_immune <- function(datafilt, scGate_DB = scGate_DB,
                        non_epi = non_epi, min_cell = 100,
                        MAGIC_impute = FALSE,
                        organism = 'human',
                        ncore = 30){
...
anno_tumor <- function(datafilt = datafilt,
  ...
  if (ncol(datafilt)>60000){
    ...
    datacanc <- infer_tumor(datafilt, cnadata = cnadata,
                           thres_sig = thres_sig,
                           thres_cor = thres_cor,
                           organism = organism,
                           ncore = ncore,
